...
import ChoixJeux from "./Component/ChoixJeux/ChoixJeux";
...
function App() {
  ...
  const [nbrChoix, setNbrChoix] = useState()
  const [afficheJeux, setAfficheJeu] = useState(true)
  const [verifierX, setVerifierX] = useState(false)
  ...
  useEffect(() => {
    if (resultat == 'YOU WIN' && verifierX) {
      setScore(score + 1)
    }
  }, [verifierX])

  const changeResultat = () => {
    if ((nbrRandom == 0 && nbrChoix == 1) || (nbrRandom == 1 && nbrChoix == 2) || (nbrRandom == 2 && nbrChoix == 3)) {
      setResultat('DRAW')
    } else if ((nbrRandom == 0 && nbrChoix == 3) || (nbrRandom == 1 && nbrChoix == 1) || (nbrRandom == 2 && nbrChoix == 2)) {
      setResultat('YOU LOSE')
    } else if ((nbrRandom == 0 && nbrChoix == 2) || (nbrRandom == 1 && nbrChoix == 3) || (nbrRandom == 2 && nbrChoix == 1)) {
      setResultat('YOU WIN')
    }
  }
  ...
  return (
    <div className="container-general">
      <button onClick={affModal} className="btn-rules">RULES</button>
      { toggle && <Modal mafonction={affModal}/> }
      <Score reset={reset} score={score}/>
      {afficheJeux ? <div className="jeu-container">
        ...
        <ChoixJeux
        functionRandom={randomNbr}
        functionX={changeVerif}
        lancer={() => leChoix(1)}
        img={imgPierre}
        choix='choix rock'
        />
      ...
      <div className="jeu-container">
        ...
        resultat={resultat} 
        img={imgPierre} 
        imgOrdi={tabOrdi[nbrRandom].img}
        choixOrdi={tabOrdi[nbrRandom].class}
        verif={verifierX}
        class='rock-step choix-step'/> } 
        { nbrChoix == 2 && <Step
        btnClick={rejouer}
        resultat={resultat} 
        img={imgFeuille}
        ...
        choixOrdi={tabOrdi[nbrRandom].class}
        verif={verifierX}
        class='scissors-step choix-step'/> } 
      </div>
      }
    </div>
  );
